...
class DataAssignAfterSuccessObserver implements ObserverInterface
{
    ...
    public function execute(Observer $observer)
    {
        ...
        if ($payment->getAdditionalInformation('stripeToken')) {
            ...
            $message = 'Authorized amount of %1. SEPA process can takes up to 14 days. Please hold the shipment until order status is processing';
            $message = __($message, $amount);
            $transaction = $payment->getAdditionalInformation('stripeToken');
        ...
        if (!empty($chargeId)) {
            ...
            $this->sepaPayment->updateChargeMetadata($chargeId, $metadata, $order);
        }
    }
}
